...
const ADMIN_ID  = parseInt(process.env.ADMIN_ID || '1743237033', 10);

if (!MONGO_URI) console.warn('⚠️  MONGO_URI not set — set it in Render environment variables');
if (!BOT_TOKEN) console.warn('⚠️  BOT_TOKEN not set — set it in Render environment variables');
...
// ── MongoDB ────────────────────────────────────────────────────────────────
// NOTE: useNewUrlParser and useUnifiedTopology were removed in Mongoose 8
...
if (bot) {
        ...
        try {
            ...
            all.forEach((u, i) => {
                const name = (u.username || u.firstName || 'Unknown')
                    .replace(/[&<>]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c]));
                msg += `${i + 1}. ${name} | <code>${u.telegramId}</code> | <b>${u.balance.toFixed(2)} TON</b> | ${u.rank}\n`;
            });
            if (msg.length > 4000) {
                await ctx.replyWithHTML(msg.slice(0, 4000));
                await ctx.replyWithHTML(msg.slice(4000));
            } else {
    ...
    // Use long-polling only — safe on Render (no webhook needed for this setup)
    bot.launch()
...
app.post('/api/bot-game', async (req, res) => {
    try {
        ...
        if (Math.random() < 0.05) {
            ...
            if (
                STATUES.every(s => user.inventory.some(i => i.statueId === s.id)) &&
                !user.completedCollections.includes('all')
            ...
            u.inventory.forEach(item => {
                ...
                if (statue && item.count > 0) {
                    items.push({
                        ...
                        statue,
                        count: item.count,
                        price: Math.floor(100 * (
                            statue.rarity === 'legendary' ? 10 :
                            statue.rarity === 'epic'      ?  5 :
                            statue.rarity === 'rare'      ?  3 : 2
                        ))
                    });
                }
            });
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', async (betAmount) => {
        try {
            ...
            if (currentRoomId && rooms[currentRoomId]) { socket.emit('error', 'Вы уже в комнате'); return; }
    ...
    socket.on('join_room', async (roomId) => {
        try {
            if (!currentUser) return;
            if (currentRoomId === roomId) return; // already here — ignore
            ...
            if (room.players.length >= 6)   { socket.emit('error', 'Комната полна');            return; }

            // Already in room (duplicate click) — just send current state
            ...
            // THIS socket gets room_joined → triggers screen switch on client
            socket.emit('room_joined', room);
            // Everyone else in the room gets room_update → re-renders player list
    ...
    socket.on('player_ready', async () => {
        try {
            ...
            if (!player || player.ready) return; // already ready — ignore duplicate
    ...
    socket.on('disconnect', () => _leaveCurrentRoom());
    ...
    try {
        ...
        setTimeout(async () => {
            try {
                ...
                if (dbWinner) {
                    io.to(room.id).emit('pvp_balance_update', {
                        telegramId: winner.user.telegramId,
                        newBalance: dbWinner.balance
                    });
                }
...
server.listen(PORT, () => console.log(`🚀 Server running on port ${PORT}`));require('dotenv').config();
const express = require('express');
const http = require('http');
const { Server } = require('socket.io');
const mongoose = require('mongoose');
...
app.post('/api/bot-game', async (req, res) => {
    try {
        ...
        res.json({
            ...
            participants: [{ name: user.username || user.firstName || 'You', color: userColor }, ...bots],
            stopAngle: Math.floor(Math.random() * 360),
            newRank: user.rank, droppedStatue, userColor
        });
    ...
    try {
        ...
        users.forEach(u => {
            u.inventory.forEach(item => {
                ...
                if (statue && item.count > 0) {
                    items.push({
                        id: `${u.telegramId}_${statue.id}`, sellerId: u.telegramId,
                        sellerName: u.username || u.firstName || 'Unknown',
...
io.on('connection', (socket) => {
    ...
    socket.on('create_room', async (betAmount) => {
        try {
            ...
            const roomId = Math.random().toString(36).substring(2, 7).toUpperCase();
            const playerColor = PLAYER_PALETTES[0][0]; // #FF2D55 for creator
            ...
            // Send room_joined ONLY to this socket so client can switch to room screen
            socket.emit('room_joined', rooms[roomId]);
            io.to('global_lobby').emit('update_rooms', getPublicRooms());
        } catch (e) { console.error('create_room error:', e); }
    });
    ...
    socket.on('join_room', async (roomId) => {
        try {
            ...
            // Already in this room — ignore duplicate clicks
            ...
            const alreadyIn = room.players.some(p => p.user.telegramId === currentUser.telegramId);
            if (alreadyIn) {
                // Just send them the current room state so they can switch screen
                socket.emit('room_joined', room);
                return;
            }
            ...
            // Send room_joined to THIS socket so it switches to room screen
            socket.emit('room_joined', room);

            // Send room_update to everyone else already in the room
            socket.to(roomId).emit('room_update', room);
    ...
    socket.on('player_ready', async () => {
        try {
            ...
            const player = room.players.find(p => p.socketId === socket.id);
            if (!player) return;
            if (player.ready) return; // already marked ready — ignore duplicate clicks
...
async function startPvPGame(room) {
    try {
        ...
        // Emit game_start to all players in the room
        io.to(room.id).emit('game_start', {
            pot: room.pot,
            players: room.players,
            playerColors: room.playerColors,
        ...
        // Wait for wheel animation (6s) then resolve winner
        setTimeout(async () => {
            try {
                const winnerIndex = Math.floor(Math.random() * room.players.length);
                const winner = room.players[winnerIndex];
                ...
                // Send game_over — client will show modal after wheel stops
                io.to(room.id).emit('game_over', {
                    winner: { ...winner.user, color: room.playerColors[winner.user.telegramId] },
                    prize: room.pot,
                    playerColors: room.playerColors
        ...
        }, 6500); // slightly longer than wheel animation (5.5s) + buffer
